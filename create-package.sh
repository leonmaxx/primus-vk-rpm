...
rpmbuild -bb "$spec_file" --define "_topdir $work_dir" --define "_rpmdir $rpm_dir" --define "_version $datestamp" --define "_driverpath $nvvulkan64"

cp *.patch $work_dir/SOURCES/

echo -e "${GREEN}Building 32-bit package...${NC}"
rpmbuild -bb "$spec_file" --target=i686 --define "_topdir $work_dir" --define "_rpmdir $rpm_dir" --define "_version $datestamp" --define "_driverpath $nvvulkan32"
